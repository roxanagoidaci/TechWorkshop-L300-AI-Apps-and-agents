...
def check_product_inventory(product_id: str) -> str:
    ...
    """
    result = inventory_check(product_id)
...
@mcp.prompt(title="Add To Cart Prompt")
def addToCartPrompt(history: str) -> str:
    prompt_template = read_prompt_file('addToCartPrompt.txt')
...
@mcp.prompt(title="Discount Fallback Prompt")
def discountFallbackPrompt() -> str:
    return read_prompt_file('discountFallback.txt')


@mcp.prompt(title="Fallback Prompt")
def fallbackPrompt() -> str:
    return read_prompt_file('fallBackPrompt.txt')


@mcp.prompt(title="Final Product Recommendation Prompt")
def finalProductRecommendationPrompt(response: str, checked_inventory: str) -> str:
    prompt_template = read_prompt_file('finalProductRecommendation.txt')
    return f"{prompt_template}\n\nresponse: {{response}}\nchecked_inventory: {{checked_inventory}}".format(response=response, checked_inventory=checked_inventory)
...
def agentPrompt(agent_name: str) -> str:
    ...
    Args:
        agent_name: One of 'cora', 'customer_loyalty', 'discount_logic', 'interior_designer', 'inventory', or 'shopper'
    """
    prompt_files = {
        "cora": "CoraPrompt.txt",
        ...
        "shopper": "ShopperAgentPrompt.txt"
    }
